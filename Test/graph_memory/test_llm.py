...
class StructuredMemoryLLMTests(unittest.IsolatedAsyncioTestCase):
    async def test_makes_exactly_one_structured_request_with_dedicated_prompt(self):
        ...
        client = StructuredMemoryLLM(settings, structured_llm=fake)
        source = ConsolidationApiRequest.model_validate(valid_request_payload()).request

        result = await client.extract(source)

        self.assertEqual(len(fake.calls), 1)
        self.assertIn("graph-memory consolidation extractor", fake.calls[0][0].content)
        self.assertIn('"schemaVersion":1', fake.calls[0][1].content)
    ...
    async def test_redacts_credentials_before_sending_observations(self):
        extraction = ConsolidationExtraction.model_validate(valid_extraction_payload())
        fake = FakeStructuredLLM(
            {"raw": None, "parsed": extraction, "parsing_error": None}
        )
        settings = MemorySettings(
            llm_config=LLMConfig(
                provider="anthropic",
        ...
        provider_message = fake.calls[0][1].content
